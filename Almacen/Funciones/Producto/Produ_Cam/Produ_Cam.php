...
#Clientes - Proveedores
		...
		if($xtipo == 'RADIAL'){
			$codigo = $clasi." ".$xmed."R".$xaro." ".$xval." ".$xmarNom." ".$xpaisNom;
		}else{
			$codigo = $clasi." ".$xmed.$xaro." ".$xval." ".$xmarNom." ".$xpaisNom;
		}
...
if (isset($_POST['Guardar'])) {
	...
	// $sql3="SELECT  (CASE WHEN MAX(produ_cam_id) = 0 THEN 1 ELSE MAX(produ_cam_id)+1 END) as codigo
	// FROM sys_produ_cam ";
	...
	// $xtipo=$_POST['xtipo'];
			...
			#Guardar Almacen
			$sql2="insert into sys_produ_cam values ('0', 
			'$xmar',
			UPPER('$xtipo'),
			'$xmed',
			'$xaro',
...
100%
	...
	echo '<META HTTP-EQUIV="refresh" CONTENT="2; URL= index.php?menu=69">';

	}elseif ($edit<>''){
		$sql1="update sys_produ_cam set 
		mar_id					= '$xmar',
		produ_cam_tp			= UPPER ('$xtipo'),
		produ_cam_med			= UPPER ('$xmed'),
		produ_cam_aro			= UPPER ('$xaro'),
		produ_cam_val			= UPPER ('$xval'),
		pais_id					='$xpais',
		produ_cam_uni			= UPPER ('$xuni'),
		produ_cam_codigo		= UPPER ('$codigo')
		where produ_cam_id='$edit'";

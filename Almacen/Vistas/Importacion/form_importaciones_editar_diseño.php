				...
				<div id="div_opc_consul" class="row" style="display:block;">
					<div class="col-lg-12">
						<p>
							...
							<a onclick="OptionStepFolio(2);" class="btn btn-sq btn-danger">
								<i class="fa fa-group fa-2x"></i><br />
								PROVEEDOR
							</a>
							<a onclick="OptionStepFolio(3);" class="btn btn-sq btn-success">
								<i class="fa fa-list fa-2x"></i><br />
								PRODUCTO
							</a>
							<a onclick="OptionStepFolio(4);" class="btn btn-sq btn-warning">
								<i class="fa fa-inbox fa-2x"></i><br />
								ORDEN
							</a>
							<a onclick="OptionStepFolio(5);" class="btn btn-sq btn-danger">
							...
							<a onclick="OptionStepFolio(6);" class="btn btn-sq btn-success">
				...
				<form id="frm_product" name="frm_product" method="post" <div class="form-group">
					<div class="row">
						...
						<div id="div_product" class="col-md-4">
							...
							<input type="hidden" id="produ_id" class="form-control">
						...
						<form id="frm_products" name="frm_products">
							<div class="table-responsive">
								<table id="tbl_importaciones_detalle" class="table table-striped table-condensed">
									...
									<tbody>
										<?php if (isset($dataProdu)) : ?>
											<?php foreach ($dataProdu as $idx => $imporProdu) : ?>
												<tr>
													<td><input type='hidden' name='produ_id_<?= $idx + 1 ?>' value='<?= $imporProdu->produ_deta_id ?>'> <?= $idx + 1 ?></td>
													<td><input type='hidden' name='product_types_<?= $idx + 1 ?>' value='<?= $imporProdu->produ_clasi_id ?>'><?= $imporProdu->clasif ?></td>
													<td><input type='hidden' name='brands_<?= $idx + 1 ?>' value='<?= $imporProdu->mar_id ?>'><?= $imporProdu->marca ?></td>
													<td><input type='hidden' name='products_<?= $idx + 1 ?>' value='<?= $imporProdu->impor_produ_nombre ?>'><?= $imporProdu->impor_produ_nombre ?></td>
													<td><input type='hidden' name='unit_price_<?= $idx + 1 ?>' value='<?= $imporProdu->impor_produ_precio_uni ?>'><?= $imporProdu->impor_produ_precio_uni ?></td>
													<td><input type='hidden' name='qty_products_<?= $idx + 1 ?>' value='<?= $imporProdu->impor_produ_cantidad ?>'><?= $imporProdu->impor_produ_cantidad ?></td>
													<td><input type='hidden' name='measure_<?= $idx + 1 ?>' value='<?= $imporProdu->produ_neu_set ?>'><?= $imporProdu->produ_neu_set ?></td>
													<td><input type='hidden' name='discounts_<?= $idx + 1 ?>' value='<?= $imporProdu->impor_produ_descuento ?>'><?= number_format($imporProdu->impor_produ_descuento, 2, ',', '.') ?></td>
													<td><input type='hidden' name='total_amount_line_<?= $idx + 1 ?>' value='<?= $imporProdu->impor_produ_precio_total ?>'><?= number_format($imporProdu->impor_produ_precio_total, 2, ',', '.') ?></td>
													<td><a class='btn btn-danger disabled'><i class='glyphicon glyphicon-trash'></i></a></td>
												</tr>
											<?php endforeach; ?>
										<?php endif; ?>
									</tbody>
									<tfoot>
										<tr>
											...
											<?php if (isset($dataProdu)) : ?>
												<?php foreach ($dataProdu as $idx => $imporProdu) : ?>
													<?php $montoTotal += $imporProdu->impor_produ_precio_total; ?>
												<?php endforeach; ?>
												<td id="amount_total_lines"><?= number_format($montoTotal, 2, ',', '.') ?></td>
											<?php else : ?>
												<td id="amount_total_lines"><?= number_format($montoTotal, 2, ',', '.') ?></td>
											<?php endif; ?>
										</tr>
										...
										<tr>
											...
											<td id="amount_total"><?= number_format($montoTotal, 2, ',', '.') ?></td>
										</tr>
									</tfoot>
								</table>
							</div>
							...
							<div class="row">
								...
								<div class="col-md-4">
								 ...
								 <div class="col-md-4">
									...
									<input type="hidden" id="saldo_pendiente" type="text" class="form-control" readonly="readonly">
								</div>
							</div>
						</form>
						<hr>

...
if ($resultadoAlmacen) {
    ...
    if (isset($_GET["fld_id"])) {
        ...
        $sqlImporProdu = "
        select prod_fld.*, brand.mar_nombre marca, brand.mar_id, prd_cls.produ_clasi_desc clasif, prd_cls.produ_clasi_id, IF(model.mod_nombre <> '',model.mod_nombre,'N/A') modelo, prd_det.*
        from sys_impor_folder fld
        inner join sys_impor_produ prod_fld on fld.impor_id = prod_fld.impor_id
        left join sys_produ_detalle prd_det on prod_fld.produ_deta_id = prd_det.produ_deta_id
        inner join sys_producto prd on prd_det.produ_deta_id = prd.produ_id
        left join sys_modelo model on prd.mod_id = model.mod_id
        inner join sys_marca brand on prd.mar_id = brand.mar_id
        inner join sys_produ_clasi prd_cls on prd.produ_clasi_id = prd_cls.produ_clasi_id
        where fld.impor_id = '". $_GET["fld_id"]."'
        ";

        //echo $sqlImporProdu;

        $resultadoProdu = mysqli_query($con, $sqlImporProdu);
        $dataProdu = array();
        if ($resultadoProdu) {
            while ($unProdu = mysqli_fetch_object($resultadoProdu)) {
                $dataProdu[] = $unProdu;
            }
        }
    }
